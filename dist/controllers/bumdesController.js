...
export class BumdesController {
    ...
    async create(req, res) {
        try {
            const { nama_bumdes, deskripsi, jenis_usaha, alamat, no_telepon, pimpinan, gambar, } = req.body;
            // Validasi input
            if (!nama_bumdes || !deskripsi || !jenis_usaha || !alamat) {
                return res.status(400).json({
                    error: "Field wajib diisi: nama_bumdes, deskripsi, jenis_usaha, alamat",
                });
            }
            ...
            if (gambar) {
                ...
                if (!gambar.startsWith("/")) {
                    return res.status(400).json({
                        ...
                        message: "Path gambar harus dimulai dengan '/' (contoh: /images/bumdes/xxx.jpg)",
                        received: gambar,
                    });
                }
                ...
                const isValidFolder = validFolders.some((folder) => gambar.includes(folder));
                if (!isValidFolder) {
                    return res.status(400).json({
                        success: false,
    ...
    async update(req, res) {
        try {
            ...
            if (updateData.gambar) {
                ...
                const isValidFolder = validFolders.some((folder) => updateData.gambar.includes(folder));
                if (!isValidFolder) {
                    return res.status(400).json({
                        success: false,
                        error: "invalid_gambar_folder",
            ...
            const updatedBumdes = await this.bumdesService.update(bumdesId, updateData);
            return res.status(200).json({
                success: true,
