  ...
  async getProfil(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const result = await pool.query(
        "SELECT id, visi, misi, sejarah, updated_at FROM profil_desa ORDER BY id DESC LIMIT 1"
      );
  ...
  async updateProfil(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const checkResult = await pool.query(
        "SELECT id FROM profil_desa ORDER BY id DESC LIMIT 1"
      );
      ...
      if (checkResult.rows.length === 0) {
        ...
        result = await pool.query(
          "INSERT INTO profil_desa (visi, misi, sejarah, updated_at) VALUES ($1, $2, $3, NOW()) RETURNING id, visi, misi, sejarah, updated_at",
          [visi, misiString, sejarah]
        );
      } else {
        // Update jika sudah ada
        const profilId = checkResult.rows[0].id;
        result = await pool.query(
          "UPDATE profil_desa SET visi = $1, misi = $2, sejarah = $3, updated_at = NOW() WHERE id = $4 RETURNING id, visi, misi, sejarah, updated_at",
          [visi, misiString, sejarah, profilId]
        );
      }
