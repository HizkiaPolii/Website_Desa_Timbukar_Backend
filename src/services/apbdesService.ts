...
export class ApbdesService {
  async getAll(): Promise<ApbdesItem[]> {
    try {
      const pool = getPool();
      const result = await pool.query(
        `SELECT id, tahun, keterangan, pendapatan, belanja, pembiayaan, 
  ...
  async getById(id: number): Promise<ApbdesItem | null> {
    try {
      const pool = getPool();
      const result = await pool.query(
        `SELECT id, tahun, keterangan, pendapatan, belanja, pembiayaan, 
  ...
  async getByTahun(tahun: number): Promise<ApbdesItem | null> {
    try {
      const pool = getPool();
      const result = await pool.query(
        `SELECT id, tahun, keterangan, pendapatan, belanja, pembiayaan, 
  ...
  async create(input: CreateApbdesInput): Promise<ApbdesItem> {
    try {
      const pool = getPool();
      const result = await pool.query(
        `INSERT INTO apbdes 
  ...
  async update(id: number, input: UpdateApbdesInput): Promise<ApbdesItem> {
    try {
      const pool = getPool();
  ...
  async getSummary(tahun: number): Promise<any> {
    try {
      const pool = getPool();
      const result = await pool.query(
        `SELECT 
