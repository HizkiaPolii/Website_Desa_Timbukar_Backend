...
export class GaleriService {
  ...
  async getAllGaleri(
    ...
    kategori?: string,
    search?: string
  ): Promise<{ data: Galeri[]; total: number }> {
    const pool = getPool();
    ...
    try {
      ...
      if (search) {
        query += ` AND (judul ILIKE $${paramIndex} OR deskripsi ILIKE $${
          paramIndex + 1
        })`;
      ...
      if (search) {
        countQuery += ` AND (judul ILIKE $${countParamIndex} OR deskripsi ILIKE $${
          countParamIndex + 1
        })`;
   ...
   */
  async getGaleriById(id: number): Promise<Galeri | null> {
    const pool = getPool();
   ...
   * Membuat galeri baru
   */
  async createGaleri(input: CreateGaleriInput): Promise<Galeri> {
    const pool = getPool();
    ...
    try {
      ...
      const result = await pool.query(
        `INSERT INTO galeri (judul, deskripsi, gambar, kategori, created_at, updated_at)
   ...
   * Update galeri
   */
  async updateGaleri(id: number, input: UpdateGaleriInput): Promise<Galeri> {
    const pool = getPool();

    try {
      // Cek apakah galeri ada
      const checkResult = await pool.query(
        "SELECT * FROM galeri WHERE id = $1",
        [id]
      );
      ...
      const query = `UPDATE galeri SET ${updates.join(
        ", "
   ...
   * Hapus galeri
   */
  async deleteGaleri(id: number): Promise<Galeri> {
    const pool = getPool();
   ...
   */
  async getKategoriList(): Promise<string[]> {
    const pool = getPool();

    try {
      const result = await pool.query(
        "SELECT DISTINCT kategori FROM galeri ORDER BY kategori ASC"
      );
